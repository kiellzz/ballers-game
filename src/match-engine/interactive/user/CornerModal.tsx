...
import { MatchOption } from "../../../components/match/MatchOption";
...
interface CornerModalProps {
  ...
  isOpen: boolean;
  onPick: (choice: CornerChoice) => void;
  onContinue: () => void;
  onClose?: () => void;
...
const BALL_PATHS: Record<Exclude<BallTrajectoryClass, "" | "corner-ball--box-then-cleared">, string> = {
...
const BALL_DURATIONS: Record<Exclude<BallTrajectoryClass, "" | "corner-ball--box-then-cleared">, number> = {
...
const BALL_ROTATIONS: Record<Exclude<BallTrajectoryClass, "" | "corner-ball--box-then-cleared">, number> = {
...
const PATH_VARIATION_PROFILES: Record<VariableTrajectoryClass, PathVariationProfile> = {
  "corner-ball--claimed": {
    c1x: 60,
    c1y: 62,
    c2x: 62,
...
function formatCubicPath(path: CubicPath): string {
  return `M ${path.mx.toFixed(1)} ${path.my.toFixed(1)} C ${path.c1x.toFixed(1)} ${path.c1y.toFixed(1)}, ${path.c2x.toFixed(1)} ${path.c2y.toFixed(1)}, ${path.ex.toFixed(1)} ${path.ey.toFixed(1)}`;
}

function varyCubicPath(
  path: string,
  profile: PathVariationProfile
): string {
...
function getVariedPath(
  trajectoryClass: VariableTrajectoryClass
): string {
...
function getBallMotionConfig(
  ...
  if (
    ...
    return {
      ...
      duration:
        trajectoryClass === ""
          ? 0.9
          : BALL_DURATIONS[trajectoryClass],
      rotation:
        trajectoryClass === ""
          ? -360
          : BALL_ROTATIONS[trajectoryClass],
      scaleValues: "1;0.85",
      scaleKeyTimes: "0;1",
    };
  }
  ...
  return {
    ...
    duration:
      BALL_DURATIONS[variableClass] +
      randomBetween(-0.15, 0.15),
    rotation:
      BALL_ROTATIONS[variableClass] +
...
export default function CornerModal({
  ...
  const [phase, setPhase] = useState<"picking" | "kicking" | "result">(
    "picking"
  );
  const [choice, setChoice] = useState<CornerChoice | null>(null);
  const [hoveredChoice, setHoveredChoice] = useState<CornerChoice | null>(null);
  ...
  const optionsLocked = phase === "picking" && choice !== null;
  ...
  function handlePick(id: CornerChoice) {
    if (phase !== "picking" || optionsLocked) return;
                ...
                : resolution?.result === "cross_bigchance"
                  ...
                  : resolution?.result === "miss"
                    ...
                    : resolution?.result === "short_kept"
                      ? "GOOD, POSSESSION KEPT!"
                      : "";

  const headerContent = shooter ? (
    <div className="corner-shooter-row">
      <span className="corner-shooter-label">Taker</span>
  ...
  const hint = phase === "picking" ? "Choose one of the 3 options" : undefined;

  const primaryAction =
    phase === "result" ? (
      <button
        className="match-modal__btn match-modal__btn--primary"
        data-testid="corner-continue-btn"
        onClick={onContinue}
      >
        Continue
      </button>
  ...
  const shortLabel = shortReceiver
    ? `Pass to ${getDisplayName(shortReceiver)}`
    : "Short";
      ...
      <div className="corner-layout">
        <div className="corner-stage">
          <img
            ...
            alt="Corner kick"
            className="corner-stage-img"
            draggable={false}
          />

          <div className="corner-stage-overlay" />

          {goalkeeper && (
            <div
              className={`corner-goalkeeper-card-wrap ${
          ...
          {shortReceiver && (
            <div
              className={`corner-short-receiver-wrap ${
          ...
          {phase === "result" && resolution && (
            <div className={`corner-banner corner-banner--${resolution.result}`}>
              {resolution.result === "goal" && "OLYMPIC GOAL!"}
              {resolution.result === "cross_claimed" && "GOALKEEPER CLAIMS IT!"}
              {resolution.result === "cross_cleared" && "CLEARED!"}
              {resolution.result === "cross_box" && "BALL INTO THE BOX!"}
              {resolution.result === "cross_bigchance" && "BIG CHANCE!"}
              {resolution.result === "miss" && "WAY OFF!"}
              {resolution.result === "short_kept" && "POSSESSION RETAINED!"}
            </div>
          )}
        </div>
        ...
        {phase === "picking" && (
          <div
            className={`corner-options-panel ${
              optionsLocked ? "corner-options-panel--locked" : ""
            }`}
          >
            {CORNER_OPTIONS.map((option) => {
              const isActive = hoveredChoice === option.id;

              return (
                <div
                  key={option.id}
                  className={`corner-option-item ${
                    isActive ? "corner-option-item--active" : ""
                  }`}
                >
                  <MatchOption
                    label={option.id === "short" ? shortLabel : option.label}
                    onClick={() => handlePick(option.id)}
                    onMouseEnter={() =>
                      !optionsLocked && setHoveredChoice(option.id)
                    }
                    onMouseLeave={() =>
                      !optionsLocked && setHoveredChoice(null)
                    }
                    type={option.id === "olympic" ? "secondary" : "primary"}
                  />
                  <span className="corner-option-sublabel">
                    {option.id === "short" ? "Short pass" : option.sublabel}
                  </span>
                </div>
              );
            })}
